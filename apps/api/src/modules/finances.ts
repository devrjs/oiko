...
export const financeRoutes = new Elysia()
  ...
  .get(
    ...
    async ({ query, session }) => {
      ...
      // Recorte temporal opcional (dashboard "Este mês"): [from, to) em ISO.
      if (query.from) {
        const from = new Date(query.from)
        if (!Number.isNaN(from.getTime())) conditions.push(gte(finances.date, from))
      }
      if (query.to) {
        const to = new Date(query.to)
        if (!Number.isNaN(to.getTime())) conditions.push(lt(finances.date, to))
      }
