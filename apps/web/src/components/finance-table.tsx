...
const FinanceRow = memo(function FinanceRow({
  finance,
  type,
}: {
  finance: Finance
  ...
  return (
    <TableRow key={finance.id} className='bg-muted/30'>
      <TableCell className='min-w-0 max-w-[180px] pr-2 pl-3 font-medium whitespace-normal break-words sm:max-w-[260px] lg:max-w-none'>
        {finance.description}
      </TableCell>
      <TableCell className={`px-2 whitespace-nowrap tabular-nums ${typeColor}`}>
        {convertToBRL(
      ...
      <TableCell className={`px-2 whitespace-nowrap ${typeColor}`}>
        {finance.type}
      </TableCell>
      <TableCell className='px-2 whitespace-nowrap tabular-nums'>
        <time dateTime={dayjs(finance.date).toISOString()}>
      ...
      <div className='flex h-full w-full flex-col'>
        {isLoading ? (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead scope='col' className='pl-3'>Descrição</TableHead>
                <TableHead scope='col' className='px-2'>Valor</TableHead>
                <TableHead scope='col' className='px-2'>Tipo</TableHead>
                <TableHead scope='col' className='px-2'>Data</TableHead>
            ...
            <TableBody>
              {data?.finances.map(finance => (
                <FinanceRow key={finance.id} finance={finance} type={type} />
              ))}
            </TableBody>
          </Table>
        )}
