...
export function Pagination({
  ...
  const lastPage = Math.ceil(
    Number(total_count_of_registers) / registers_per_page
  )
  const previousPages =
    current_page > 1
  ...
  return (
    <nav
      ...
      className='flex max-h-[56px] min-h-[56px] items-center justify-center gap-2'
    >
        ...
        <>
          ...
          {current_page > 2 + siblingsCount && (
            <span className='text-muted-foreground'>
              ...
              <span className='sr-only'>Páginas omitidas</span>
            </span>
          )}
        </>
      )}
